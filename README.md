...
全站 97 个组件、9 大章节，没有任何 UI 组件库——每一个组件（按钮、命令面板、看板拖放、日期选择器……）都是用 React + Tailwind CSS 手写实现的，这也是本站的意义：教你从零理解每个组件的原理。
...
| 02 基础组件与表单 | 按钮、输入框、滑块、开关、OTP 验证码、日期选择器、标签输入等 |
| 03 导航 | 顶部导航栏、标签页、命令面板 ⌘K、右键菜单、macOS Dock、底部标签栏 |
| 04 反馈与覆盖层 | Toast、对话框、抽屉、警告条、进度指示、骨架屏、悬浮操作按钮 |
| 05 数据展示 | 表格、树形视图、看板拖放、日历热力图、轮播、图片对比、瀑布流 |
| 06 指针交互与背景 | 聚光灯背景、鼠标视差、音频律动条、3D 倾斜、粒子连线、毛玻璃 |
| 07 文字、滚动与高级模式 | 跑马灯、打字机、数字滚动、拖拽排序、左滑删除、主题切换转场 |
| 08 模式与原则 | 布局骨架、响应式策略、表单体验、深色模式方法论、设计走查清单 |
